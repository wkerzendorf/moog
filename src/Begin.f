...
c***************************************************************************
      ...
      character*80 barklemPath, barklemUVPath, moogDataPath
...
c    Looking for environmental variable moogdata
      call getenv('moogdata', moogDataPath)
      
      if (moogDataPath .eq. '') then
          moogDataPath = '/usr/local/share/moog/'
      endif
...
c      print*,'Current moogDataPath ',moogDataPath

 
      barklemPath = moogDataPath
      barklemPath = trim(barklemPath) // 'Barklem.dat'

      open (nfbarklem,file=barklemPath)

 
c*****open data files carried with the source code: Barklem UV damping
      nfbarklemUV = 36
      
      barklemUVPath = moogDataPath
      barklemUVPath = trim(barklemUVPath) // 'BarklemUV.dat'
      print*,fbarklemUV
      open (nfbarklemUV, file=barklemUVPath)
